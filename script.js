...
api_key='079dad34a7916d3ffffbeed9e6bc128d',
currentWeatherCard = document.querySelectorAll('.weather-left .card')[0],
sunCard = document.getElementById('sun-card'),
aqiCard = document.querySelectorAll('.highlights .card')[0],
fiveDaysForecastCard = document.getElementById('day-forecast'),
...
hourlyForecastCard = document.getElementById('hr-forecast');

function getWeatherDetails(name , lat, lon, country, state){
    let FORECASTE_API_URL = `https://api.openweathermap.org/data/2.5/forecast?lat=${lat}&lon=${lon}&appid=${api_key}`;
    ...
    fetch(AIR_POLLUTION_API_URL).then(res => res.json()).then(data => {
        let {co, no, no2, o3, pm2_5, pm10, so2, nh3} = data.list[0].components;
        aqiCard.innerHTML = `
        <div class="card-head">
    ...
    fetch(FORECASTE_API_URL).then(res => res.json()).then(data =>{
